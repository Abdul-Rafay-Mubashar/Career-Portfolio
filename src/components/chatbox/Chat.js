...
export default function Chat() {
  ...
  const [messages, setMessages] = useState([{ sender: "assistant", text: "Hi! I am here to assist. Ask anything about Rafay?" }]);
  const [input, setInput] = useState("");
  const [typing, setTyping] = useState(false);
  const [conversation, setConversation] = useState([{ role: "assistant", content: "I am here to assist. Ask anything about Rafay?" }]);
  ...
  const askOpenAI = async (question) => {
    ...
    const memoryText = newConversation.map((m) => `${m.role}: ${m.content}`).join("\n");
...
`;
  ...
  {!isOpen && <div className="chat-hint">Lets talk about Rafay!</div>}
      ...
      <div className='box'>
        ...
        {typing === true?<div className="person-a">
          <div className="icon"></div>
